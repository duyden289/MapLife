...
typedef NS_ENUM(NSInteger, MServiceHTTPBase)
{
    MServiceHTTPBaseOK = 200
};
/**
 * Key API URL Request
 */
...
- (void)startRequest:(NSURLRequest *)request
{
    ...
    self.urlSessionDataTask = [session dataTaskWithRequest:request completionHandler:^(NSData * _Nullable data, NSURLResponse * _Nullable response, NSError * _Nullable error) {
...
- (void)requestCompleteWithData:(NSData *)data response:(NSURLResponse *)response error:(NSError *)error
{
    if (error != nil) {
    ...
    else
    {
        ...
        if (httpResponse.statusCode == MServiceHTTPBaseOK ) {
            ...
            if ([self.delegate respondsToSelector:@selector(requestDidReviceNormalData:)]) {
        ...
        {
            if ([self.delegate respondsToSelector:@selector(requestDidReviceErrorFromServer:)]) {
